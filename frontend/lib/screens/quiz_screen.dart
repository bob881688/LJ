...
class _QuizScreenState extends State<QuizScreen> {
  ...
  */
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("測驗中 (${widget.mode == 'word' ? '單字' : '句子'} $currentQuestionIndex/$totalQuestions)"),
      ),
      body: isLoading
        ...
        child: Column(
          ...
          children: [
            const Text("連線失敗，請檢查後端", style: TextStyle(color: Colors.white)),
            const SizedBox(height: 20),
            ElevatedButton(onPressed: loadNewQuestion, child: const Text("重試"))
          ],
        ),
      )
          : Padding(
                  ...
                  child: ElevatedButton(
                    style: ButtonStyle(
                      backgroundColor: WidgetStateProperty.resolveWith<Color>((states) {
                        ...
                        if (answered) {
                          ...
                          // 如果所選是對的，其餘全部灰色。
                          if (!selectedWasCorrect && option['is_correct'] == true) {
                            return Colors.green;
                          }
                          return Colors.grey;
                        }
                    ...
                    onPressed: answered
                      ...
                      setState(() {
                        answered = true;
                        selectedIndex = index;
                        selectedWasCorrect = option['is_correct'] == true;
                        if (option['is_correct']) score++;

                        // 記錄這一題的詳細狀況
                        String correctText = "未知";
                        try {
                          var correctOption = currentQuiz!['options'].firstWhere(
                                  (o) => o['is_correct'] == true,
                              orElse: () => null
                          );
                      ...
                      Future.delayed(const Duration(seconds: 1), loadNewQuestion);
                    },
                    child: Text(
                      option['text'],
